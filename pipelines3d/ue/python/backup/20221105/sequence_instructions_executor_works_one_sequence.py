...
class SequenceInstructionsExecutor:
    ...
    def process_sequence_instructions(self, sequence_instructions):
        for scene_data in sequence_instructions:
            ...
            for (start_frame, end_frame) in scene_data["render_cuts"]:
                self._render_cut(scene_sequence, scene_name=scene_name, start_frame=start_frame, end_frame=end_frame, fps=fps)
                break
    ...
    def _render_cut(self, scene_sequence, scene_name, start_frame, end_frame, fps):
        print('\n' * 5)
        print('*************')
        if False:
            while lf.ue_context.get_rendering_lock():
                print(f' - Waiting for acquire rendering lock to be released {dt.datetime.now()}')
                time.sleep(10)
        print(' - Trying to acquire rendering lock')
        lf.ue_context.set_rendering_lock(True)
        print(f'    - Rendering cut {start_frame, end_frame}')
        self._set_sequence_range(scene_sequence, start_frame=start_frame, end_frame=end_frame, fps=fps)
        output_path = f"{self.OUTPUT_DIR}/{scene_name}_{start_frame}_{end_frame}"
        self._render_scene_sequence(scene_sequence, output_path)
    ...
    def _render_scene_sequence(self, scene_sequence, output_path):
        ...
        #
        # def render_sequence_local():
        #     print('Started thread')
        #     scene_sequence_path = lf.get_asset_path(scene_sequence)
        #     lf.render_sequence_to_images(scene_sequence_path, output_path)
        # t = threading.Thread(target=render_sequence_local)
        # t.run()
        #
