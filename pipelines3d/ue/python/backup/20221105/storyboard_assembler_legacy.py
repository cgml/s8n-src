...
class StoryboardCompositorLegacy:
    ...
    def convert_to_nn_types(self, original_shot_types):
        result = []
        for st in original_shot_types:
            if self.SHOT_TYPES.get(st) != None:
                result.append(self.SHOT_TYPES.get(st)['nn_match'])
            else:
                print('Not found', st)
        return result

    def extract_sequence_from_manual_analysis(self, manual_analysis_csv_path):
        st = []
        with open(manual_analysis_csv_path) as f:
            lines = f.read().split('\n')
        for line in lines:
            st.append(line.split(',')[2])
        result = self.convert_to_nn_types(st)
        return result
