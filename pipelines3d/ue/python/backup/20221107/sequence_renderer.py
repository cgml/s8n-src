...
class SequenceRenderer:
    ...
    def process_render_queue(self, final_callback, resolution_width=1920, resolution_height=1080, spatial_sample_count=16):
        ...
        # def process_next_callback(success):
        #     print(f"*************: process_next {success}")
        #     process_next()

        def process_next():
            # m_callback = unreal.OnRenderMovieStopped()
            # m_callback.bind_callable(process_next_callback)
            ...
            print('\n' * 5)
            print('*************')
            # app_context.set_rendering_lock(True)
